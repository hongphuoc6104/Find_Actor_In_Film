...
| **2** | Thiết kế kiến trúc hệ thống: pipeline xử lý 9 giai đoạn |
...
**Ý nghĩa:** Giá trị của hệ số Silhouette ($S$) nằm trong khoảng $[-1, 1]$. Nếu $S$ tiến gần đến 1, điều này chỉ ra rằng điểm dữ liệu được gán đúng vào cụm phù hợp, với khoảng cách nội cụm nhỏ (độ nén cao) và khoảng cách liên cụm lớn (độ tách biệt tốt). Giá trị $S$ xấp xỉ 0 cho thấy điểm dữ liệu nằm trên hoặc rất gần ranh giới quyết định giữa hai cụm lân cận. Ngược lại, nếu $S$ nhận giá trị âm, điểm dữ liệu đó có khả năng đã được gán sai vào cụm.
...
**Nguồn gốc:** Được đề xuất trong [15].
...
**Nguồn gốc:** Được đề xuất trong [16].
...
**Tham số quan trọng:**
**Tham số quan trọng:** Phương pháp t-SNE có ba tham số chính ảnh hưởng đến kết quả hiển thị. Đầu tiên là `perplexity`, đại diện cho số lượng láng giềng hiệu quả, thường được đặt trong khoảng 30-50 để cân bằng giữa cấu trúc cục bộ và toàn cục. Thứ hai là `n_iter` (số vòng lặp tối ưu), thường được thiết lập ở mức 1000 để đảm bảo thuật toán hội tụ. Cuối cùng, `learning_rate` quyết định tốc độ cập nhật của thuật toán, với giá trị phổ biến là 200.
...
**Tham số quan trọng:**
**Tham số quan trọng:** Tương tự như t-SNE, UMAP cũng có các tham số điều khiển cấu trúc hình học của dữ liệu. `n_neighbors` (số lượng láng giềng) kiểm soát sự cân bằng giữa cấu trúc cục bộ và toàn cục, thường chọn từ 15 đến 50. `min_dist` quy định khoảng cách tối thiểu giữa các điểm trong không gian chiếu 2D, với giá trị nhỏ (0.1-0.5) giúp các cụm hiển thị gọn hơn. Quan trọng nhất, tham số `metric` cần được thiết lập là `cosine` để đồng nhất với độ đo khoảng cách được sử dụng trong quá trình phân cụm.
...
Về tốc độ tính toán, UMAP vượt trội hơn với độ phức tạp $O(n \log n)$, nhanh hơn đáng kể so với độ phức tạp $O(n^2)$ của t-SNE, làm cho UMAP phù hợp hơn với các bộ dữ liệu lớn. Về khả năng bảo toàn cấu trúc, trong khi t-SNE làm rất tốt việc bảo toàn cấu trúc cục bộ nhưng kém trong việc thể hiện cấu trúc toàn cục, UMAP lại cân bằng tốt cả hai yếu tố này, giúp khoảng cách giữa các cụm trên biểu đồ phản ánh chính xác hơn mối quan hệ thực tế. Ngoài ra, UMAP có tính ổn định cao hơn, kết quả ít bị thay đổi ngẫu nhiên qua các lần chạy so với t-SNE. Do đó, UMAP đang ngày càng trở nên phổ biến và thay thế dần t-SNE trong nhiều ứng dụng thực tế.
...
**Đầu ra của RetinaFace:**
Mô hình RetinaFace trả về ba thông tin quan trọng cho mỗi khuôn mặt phát hiện được: (1) **Khung bao (Bounding box)** được xác định bởi tọa độ góc trên trái và dưới phải, dùng để cắt ảnh khuôn mặt; (2) **Điểm tin cậy (Confidence score)** trong khoảng [0, 1] cho biết xác suất đây là một khuôn mặt thực sự; và (3) **5 điểm mốc (Landmarks)** tương ứng với vị trí mắt trái, mắt phải, mũi, khóe miệng trái và khóe miệng phải, đóng vai trò cốt lõi cho bước căn chỉnh khuôn mặt (face alignment) sau này.
...
Để đảm bảo độ chính xác cho quá trình nhận diện, hệ thống áp dụng các ngưỡng lọc nghiêm ngặt. **Điểm phát hiện (Detection Score)** phải đạt tối thiểu 0.45 để loại bỏ các trường hợp nhận diện sai. **Kích thước khuôn mặt** cần lớn hơn $50 \times 50$ điểm ảnh để đảm bảo đủ chi tiết cho trích xuất đặc trưng. **Điểm độ nét (Blur Score)**, được tính bằng phương sai Laplacian, phải lớn hơn 40.0 để loại bỏ ảnh quá mờ. Cuối cùng, **Điểm góc nghiêng** được dùng để loại bỏ các khuôn mặt có góc quay quá lớn, đảm bảo khuôn mặt đủ trực diện cho mô hình ArcFace xử lý.
...
| scikit-learn [17] | 1.3.0 | Agglomerative Clustering, metrics |
...
| faiss-cpu [18] | 1.7.4 | Vector similarity search |
